...
export const getMessages = (name, sex) => [
  {
    ...
    text: () => (
      <TextSm>
        Привет, {name}!
        <TextDivider />
        Поздравляю с{"\u00A0"}началом первого рабочего дня! А{"\u00A0"}у
        {"\u00A0"}меня уже есть для тебя задание. Твой клиент — продавец очень
        красивых и{"\u00A0"}удобных диванов. В{"\u00A0"}последнее время его
        продажи на{"\u00A0"}нашей платформе упали, и{"\u00A0"}нам кажется, что
        ему нужна помощь.{" "}
        <UnderlinedText color={"green"}>
          Узнай, что случилось, и{"\u00A0"}предложи решение этой проблемы.
        </UnderlinedText>{" "}
        Весь офис, коллеги и{"\u00A0"}я в{"\u00A0"}твоем распоряжении. Мы в
        {"\u00A0"}тебя верим!
      </TextSm>
    ),
    type: "interlocutor",
...
export const getQuestionMessages = (questionNumber, sex) => [
  ...
  },
  {
    ...
    text:
      questionNumber === "1"
        ? "Я могу обратиться за помощью к коллегам?"
        : "Я буду работать только с продавцом диванов или у меня будут и другие клиенты?",
    type: "main",
    delay: 0,
    avatar: sex === SEX_TYPES.female ? playerF : player,
  },
...
export const getPersonMessages = (sex) => [
  {
    ...
    color: "red",
    text:
      "Только что вернулся от\u00A0нашего массажиста. " +
  ...
  },
  {
    ...
    color: "red",
    text:
      "Очень даже хватает. Мне нравится заниматься там йогой, например. " +
      "А если не любишь йогу, можно устроить силовую тренировку. Попробуй как-нибудь!",
    delay: 2200,
  },
];
